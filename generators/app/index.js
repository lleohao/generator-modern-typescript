...
module.exports = class extends Generator {
  ...
  prompting() {
    ...
    this.log(
      yosay(
        `Welcome to the doozie ${chalk.red('generator-modern-typescript')} generator!`
      )
    );
  ...
  writing() {
    ...
    this.fs.copy(this.templatePath('tsconfig.json'), resolvePath('tsconfig.json'));

    if (useJest) {
      this.fs.copy(this.templatePath('src'), resolvePath('src'));
      this.fs.copy(this.templatePath('jest.config.js'), resolvePath('jest.config.js'));
    } else {
      this.fs.copy(this.templatePath('src/index.ts'), resolvePath('src/index.ts'));
      this.fs.copy(
        this.templatePath('src/tsconfig.dev.ts'),
        resolvePath('src/tsconfig.dev.ts')
      );
    }

    if (useTslint) {
      this.fs.copy(this.templatePath('tslint.json'), resolvePath('tslint.json'));
    }
